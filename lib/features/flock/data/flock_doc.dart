...
class FlockDoc {
  ...
  factory FlockDoc.fromMap(String id, Map<String, dynamic> m) => FlockDoc(
        id: id,
        ...
        memberUids: List<String>.from(m['memberUids'] ?? const []),
        memberNames: List<String>.from(m['memberNames'] ?? const []),
  ...
  Flock toFlock() => Flock(
        ...
        members: [
          for (var i = 0; i < memberNames.length; i++)
            Person(memberNames[i], verified: i == 0 && verifiedHost),
        ],
        lat: lat,
        lng: lng,
      );
